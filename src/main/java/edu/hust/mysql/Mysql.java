...
public class Mysql {
    ...
    public String getTblSQL(String tableName) {
        ...
        String sql = new String("create table if not exists " + tableName + "(time int not null primary key,"
            +"value"+" float not null"+") ENGINE=INNODB DEFAULT CHARSET=utf8;");

        return sql;
    }


    public void insertBenchMark(FloatData[] floatData,String tableName,int id){
        ...
        for ( int i = 0;i<floatData.length;i++) {
            if (floatData[i].flag == 0)
                continue;
            suffix.append("(" + floatData[i].tm + "," + floatData[i].val +  "),");
